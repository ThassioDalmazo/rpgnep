...
export const DMTools: React.FC<Props> = ({ encounter = [], setEncounter, logs = [], addLog, characters = [], monsters = [], setMonsters, turnIndex, setTurnIndex, targetUid, setTargetUid, compact = false, onAddCombatant }) => {
  ...
  const rollAttack = (attackerName: string, actionName: string, hitBonus: number, dmg: string) => {
    ...
    // CRITICAL FIX: Ensure hitBonus is a number to prevent string concatenation (e.g., 10 + "5" = "105")
    const totalHit = d20 + Number(hitBonus);
    const isCrit = d20 === 20;
    const isFail = d20 === 1;
    ...
    if (target) {
        ...
        else if (totalHit >= Number(target.ac || 10)) { 
    ...
    addLog(
        `${attackerName} usa ${actionName}`, 
        `${hitResult ? `>> ${hitResult} <<\n` : ''}Acerto: ${d20}${rollDetails} + ${hitBonus} = ${totalHit}\nDano: ${dmgTotal} ${diceRolledStr}`, 
        hitColor
    );
  };
        ...
        {(showLogs || !compact) && (
            <div className={`${compact ? 'h-1/4 border-t' : 'w-72 border-l'} bg-[#161619] border-[#2a2a2e] flex flex-col shadow-xl z-10 shrink-0`}>
                ...
                <div className="flex-1 overflow-y-auto p-2 space-y-2 custom-scrollbar bg-[#121212]">
                    {logs.map(log => (
                        <div key={log.id} className={`p-2 rounded border text-[10px] leading-snug ${
                            ...
                            (log.details && log.details.includes && log.details.includes('ACERTA')) ? 'bg-green-900/20 border-green-600/50 text-green-300' :
                            'bg-[#1f1f23] border-[#333] text-stone-400'
                        }`}>
