    ...
    <div className="flex flex-col h-full bg-[#121212] text-stone-200 font-sans overflow-hidden">
              ...
              <div className="bg-[#1a1a1d] border border-stone-700 rounded-xl w-full max-w-3xl h-[90vh] flex flex-col shadow-2xl relative overflow-hidden">
                  ...
                  <div className="flex-1 overflow-y-auto p-6 space-y-6 custom-scrollbar bg-[#121212]">
                      ...
                      <div className="space-y-1">
                          ...
                          <input className="w-full bg-[#222] border border-[#333] rounded p-2 text-white text-xs" value={editingMonster.imageUrl || ''} onChange={e => setEditingMonster({...editingMonster, imageUrl: e.target.value})} placeholder="https://..." />
                      </div>
                  </div>
      ...
      <div className={`flex flex-1 overflow-hidden ${compact ? 'flex-col relative' : 'flex-row'}`}>
            ...
            <div className={`${compact ? 'absolute inset-0 z-10 bg-[#121212]' : 'w-80 border-r'} border-[#2a2a2e] flex flex-col p-3 shadow-xl overflow-hidden`}>
                ...
                {libraryTab === 'bestiary' && ( 
                    <div className="flex gap-2 mb-2">
                        <button onClick={createEmptyMonster} className="flex-1 py-1.5 bg-stone-800 border border-stone-600 rounded text-xs font-bold text-stone-300 hover:text-white hover:border-stone-400 flex items-center justify-center gap-2"><Plus size={14}/> Manual</button>
                        <button onClick={() => setAiPromptOpen(true)} className="flex-1 py-1.5 bg-gradient-to-r from-purple-900/40 to-amber-900/40 border border-purple-500/30 rounded text-xs font-bold text-purple-300 hover:text-white hover:border-purple-500 flex items-center justify-center gap-2"><BrainCircuit size={14}/> IA</button> 
                    </div>
                )}
                    ...
                    {libraryTab === 'bestiary' && groupedMonsters.map(group => (
                        <div key={group.cr}>
                            ...
                            {group.items.filter(m => m.name.toLowerCase().includes(searchTerm.toLowerCase())).map(m => (
                                <div key={m.id} className="bg-[#222226] p-1.5 rounded border border-[#333] hover:border-stone-500 flex justify-between items-center mb-1 group">
                                    <div className="flex items-center gap-2">
                                        <div className="w-6 h-6 bg-black rounded overflow-hidden border border-[#333]">
                                    ...
                                    <div className="flex gap-1">
                                        <button onClick={() => setEditingMonster(m)} className="text-stone-600 hover:text-blue-400 p-1 bg-black/30 rounded hover:bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity"><Pencil size={12}/></button>
        ...
        <div className={`flex-1 bg-[#0c0c0e] p-2 md:p-4 overflow-y-auto custom-scrollbar relative flex flex-col ${compact ? 'absolute inset-0 z-0' : ''}`}>
           {/* ... (Existing Tracker Content remains unchanged) ... */}
